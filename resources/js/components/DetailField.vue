    ...
    <panel-item :field="field">
        ...
        <template slot="value">
            <a
                ...
                target="_blank"
                class="text-primary inline-flex items-center dim cursor-pointer no-underline"
            >
                <span v-if="isLinkedIn" class="text-primary font-bold">@{{ getLinkedInName }}</span>
                <span v-else class="text-primary font-bold">@{{ field.value }}</span>
            </a>
            <p v-else>&mdash;</p>
        </template>
    </panel-item>
...
export default {
    ...
    computed: {
        ...
        getLinkedInName() {
            const name = this.field.value.split('-');
